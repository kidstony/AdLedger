...
- [x] **Lộ trình test camp mới** [`launch-checklist.ts`](src/lib/launch-checklist.ts) + [`LaunchChecklist.tsx`](src/components/optimize/LaunchChecklist.tsx): hướng dẫn 3 giai đoạn cho camp non (chặn rác → tín hiệu sớm → chờ chín); checklist auto-check (sync, bid strategy sai khi không conversion, Presence, broad, term rác, nhập DT) + 3 mục hướng dẫn manual; **stop-loss** `projects.test_budget` ([`migration_test_budget.sql`](supabase/migration_test_budget.sql)) — chi lũy kế từ camp_start_date chạm mức mà DT lũy kế = 0 → báo đỏ DỪNG TEST + thẻ cut high. Hiện cả khi camp chưa có metrics; tự ẩn khi trưởng thành.
- [x] **Plain-language UX**: khối "Làm gì tiếp theo" (`NextSteps.tsx` — top-3 hành động + nơi bấm trong Google Ads); meta/detail Insight Miner viết thành câu thường ("chiếm 40% chi phí ngày lãi, chỉ 5% ngày lỗ"); tooltip thuật ngữ ở scorecard (DT Màn hình/ROI/CTR/CPC/IS) + chú giải share/pp ở bảng lift.
