// Chuẩn hóa dimension từ network affiliate về giá trị chung (bản TypeScript cho Next.js):
// - country → ISO-3166 alpha-2 UPPER ('US', 'VN'...) — khớp map geo criterion của Google Ads
// - device  → 'mobile' | 'desktop' | 'tablet' | 'other'
// LƯU Ý: đây là MIRROR của engine/lib/normalize.js (engine ESM JS không import được TS và
// ngược lại) — sửa bảng ở đây thì sửa cả bên đó. Detect route dùng bản này để nhận diện
// cột country/device khi dò config; engine dùng bản JS khi chạy thật → phải cho kết quả giống nhau.
...
const NAME_TO_2 = new Map<string, string>()
...
export function normalizeCountry(value: unknown, valueMap: Record<string, string> | null = null): string {
...
export function normalizeDevice(value: unknown, valueMap: Record<string, string> | null = null): string {
  if (value === null || value === undefined) return ''
  const raw = String(value).trim()
  if (!raw) return ''
  const lower = raw.toLowerCase()
...
// MIRROR engine/lib/normalize.js — detect route dùng bản này để nhận cột text nhúng nước/thiết bị.
...
// text hỗn hợp → alpha-2 UPPER. Thử normalizeCountry (giá trị sạch) trước; rồi quét tên nước nhúng.
export function extractCountryFromText(value: unknown, valueMap: Record<string, string> | null = null): string {
...
// text → 'mobile'|'desktop'|'tablet'|'' (bỏ 'other'). normalizeDevice sạch trước; rồi quét từ khóa.
export function extractDeviceFromText(value: unknown, valueMap: Record<string, string> | null = null): string {
