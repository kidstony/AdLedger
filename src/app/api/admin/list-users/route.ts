...
export async function GET(req: Request) {
  ...
  let profileQuery = supabaseAdmin
    .from('user_profiles')
    .select('user_id, full_name, role, team_id, organization_id, teams(id, name, color)')

  if (caller.role === 'manager' && caller.team_id) {
    profileQuery = profileQuery.eq('team_id', caller.team_id)
  }

  if (caller.role === 'super_admin' && caller.organization_id) {
    profileQuery = profileQuery.eq('organization_id', caller.organization_id)
  }
  ...
  const users = authData.users
    ...
    .map(u => {
      const profile = profileMap.get(u.id) as { user_id: string; full_name: string; role: string; team_id: string | null; organization_id: string | null; teams: Record<string, string> | null } | undefined
      return {
        user_id:         u.id,
        email:           u.email ?? '',
