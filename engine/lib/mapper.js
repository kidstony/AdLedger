...
// Map 1 dòng raw của report breakdown → { row } hoặc { error }.
...
function mapBreakdownRow(raw, mapping, dimensions, tz, windowEndDate = null) {
  ...
  if (rev === undefined || rev === null || rev === '') rev = mapping.revenue.default
  ...
  if (ct?.path) {
    ...
    const dt = parseDateTime(v, ct.formats ?? [], ct.timezone ?? tz ?? null)
    if (dt) {
      out.hour = dt.hour
...
// Map toàn bộ raw rows của report breakdown → { mapped, invalid, errorSamples }
