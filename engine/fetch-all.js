...
import { mapRows, dedupeRows, toPnlRows } from './lib/mapper.js'
import { insertRun, updateRun, upsertRevenueRaw, upsertAffiliateRevenue, upsertNetwork } from './lib/db.js'
import { loadAccounts } from './lib/accounts.js'
import { openAlert, closeAlerts } from './lib/alerts.js'
import { dateWindow } from './lib/dates.js'
import { getRateToUsd } from './lib/fx.js'
...
async function runNetwork(config, dryRun, accountFilter = null) {
...
async function runAccount(config, account, dryRun) {
  ...
  try {
    ...
    result = await captureReports(page, config)
  } catch (err) {
...
async function main() {
  ...
  const configs = loadConfigs(args.network) // fail fast nếu config lỗi
